...
docker compose -f configs/infra/docker-compose.yml down --remove-orphans
...
docker compose -f configs/infra/docker-compose.yml up -d --build
echo "[4/4] [FINISHED] Initialized Docker Infrastructure."

echo "[5/5] [START] Tailing logs (Press Ctrl+C to stop watching)..."
docker compose -f configs/infra/docker-compose.yml logs -f
